...
from nawiri.nawiri_company  import getParameterInfo
...
def addPosReceiptDetails(
    ...
    uuid.uuid4(), receipt_id, datetime.datetime.now(), staff_id, shift_id, till_id, "N", receipt_total_amount,)
    ...
    for item in cartItem:
        ...
        data = (uuid.uuid4(), branch_id, 'N', '0', count, product["location_product_id"],
                product["product_bp"], product["location_product_sp"], receipt_id, float(item["quantity"]),
                product["uom_code"], 'N',)
        cursor.execute(query, data)
